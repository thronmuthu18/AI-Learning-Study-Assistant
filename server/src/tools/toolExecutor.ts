  ...
  public static async executeTool(
    ...
    switch (name) {
      ...
      case 'getUserProgress': {
        const totalCourses = await Course.countDocuments({ userId: userObjectId });
        const completedTasks = await LearningTask.countDocuments({
          userId: userObjectId,
          status: 'completed',
        });
        const totalTasks = await LearningTask.countDocuments({ userId: userObjectId });
        const attempts = await QuizAttempt.find({ userId: userObjectId }).sort({ createdAt: -1 });
      ...
      case 'getLearningPlan': {
        const filter: any = { userId: userObjectId, status: 'active' };
        if (args.courseId) filter.courseId = new mongoose.Types.ObjectId(args.courseId);

        const plan = await LearningPlan.findOne(filter).sort({ updatedAt: -1 });
        if (!plan) {
          return {
            success: true,
            data: null,
        ...
        const tasks = await LearningTask.find({ planId: plan._id })
      ...
      case 'recommendNextTopic': {
        const recommendations = await RecommendationService.generateRecommendations(userObjectId);
        return {
          success: true,
