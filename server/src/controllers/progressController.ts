...
export const getProgressDashboard = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const totalDocuments = await DocumentModel.countDocuments({ userId, status: 'ready' });
  const totalPlans = await LearningPlan.countDocuments({ userId });

  // 2. Learning Plan Task Progress
  const totalTasks = await LearningTask.countDocuments({ userId });
  const completedTasks = await LearningTask.countDocuments({ userId, status: 'completed' });
  const taskCompletionRate = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0;

  // Active Plan & Today's tasks
  const activePlan = await LearningPlan.findOne({ userId, status: 'active' })
    .populate('courseId', 'title color icon')
    .sort({ updatedAt: -1 });

  let upcomingTasks: any[] = [];
  if (activePlan) {
    upcomingTasks = await LearningTask.find({
  ...
  const highestQuizScore =
    totalQuizzesTaken > 0 ? Math.max(...quizAttempts.map((a) => a.percentage)) : 0;

  // Aggregate Weak & Strong Topics
  const topicErrorCounts: Record<string, { total: number; correct: number }> = {};
  for (const attempt of quizAttempts) {
    for (const ans of attempt.answers) {
      ...
      if (ans.isCorrect) topicErrorCounts[topic].correct += 1;
    }
  }

  const weakTopics: Array<{ topic: string; accuracy: number; questions: number }> = [];
  const strongTopics: Array<{ topic: string; accuracy: number; questions: number }> = [];
  ...
  const sessions = await StudySession.find({ userId }).sort({ activityDate: -1 });
  ...
  const recentDocuments = await DocumentModel.find({ userId, status: 'ready' })
 ...
 */
function generateLast7DaysActivity(sessions: any[]): Array<{ day: string; date: string; minutes: number; tasks: number }> {
  const result: Array<{ day: string; date: string; minutes: number; tasks: number }> = [];
